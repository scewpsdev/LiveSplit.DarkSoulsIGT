...
namespace LiveSplit.DarkSoulsIGT
{
    internal static class DSMemory
    {
        ...
        public static IntPtr Scan(Process process, Dictionary<IntPtr, byte[]> readMemory, byte?[] aob)
        {
            ...
            if (results.Count == 0)
            {
                throw new ArgumentException(string.Format(DSConfig.AOBNotFound, aob.ToString()));
            }
            else if (results.Count > 1)
            {
                throw new ArgumentException(string.Format(DSConfig.AOBMultiple, results.Count, aob.ToString()));
            }
            return results[0];
        }
